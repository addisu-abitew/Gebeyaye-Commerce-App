...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Gebeyaye-Server account.<br>
        ...
        &mdash; Gebeyaye-Server Team
      `
      return sendMail({ toEmail: email, subject: 'Gebeyaye-Server - Password Reset', content })
    })
